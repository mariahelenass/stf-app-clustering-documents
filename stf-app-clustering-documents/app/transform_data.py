...
# stopwords
with open("ml/stopwords_br.txt", "r", encoding="utf-8") as f:
    stop_words = set([x.strip().lower() for x in f.readlines()])

with open("ml/stopwords_juridicas.txt", "r", encoding="utf-8") as f:
    juridicas = set([x.strip().lower() for x in f.readlines()])

with open("ml/topicos_comuns.txt", "r", encoding="utf-8") as f:
...
class TransformData:
    ...
    def normalize_citations(text: str) -> str:
        ...
        return re.sub(r"\binciso\s+([ivx]+)\b", roman_to_num, text)
    ...
    def remove_ocr_noise(text: str) -> str:
        clean = []
        for w in text.split():
            w_norm = TransformData.normalize_token(w)
    ...
    def preprocess(text: str) -> str:
        ...
        text = TransformData.normalize_spacing(text)
        text = TransformData.general_cleanup(text)
        text = TransformData.remove_accents(text).lower()
        text = TransformData.normalize_citations(text)
        text = TransformData.normalize_spacing(text)
        ...
        text = " ".join(words)
        text = TransformData.remove_ocr_noise(text)
